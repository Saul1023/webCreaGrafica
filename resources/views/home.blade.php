...
<main>
    <section id="productos" class="py-24 bg-gray-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @php
            // Consulta rápida temporal para mostrar los productos activos en la tienda sin romper nada
            $productosPublicos = \App\Models\Producto::where('activo', true)->orderBy('id', 'desc')->take(6)->get();
            @endphp

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($productosPublicos as $prod)
                <div
                    class="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all overflow-hidden border border-gray-100 group">
                    <div class="relative overflow-hidden bg-gray-100 h-64 flex items-center justify-center">
                        ...
                        @if($prod->tiene_3d)
                        <span
                            class="absolute top-3 right-3 bg-purple-600 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-sm flex items-center gap-1">
                    ...
                    <div class="p-6">
                        <span
                            class="text-xs font-bold text-blue-600 tracking-wide uppercase">{{ $prod->categoria->nombre ?? 'Sublimación' }}</span>
                        <h3 class="text-xl font-bold text-gray-900 mt-1 mb-2">{{ $prod->nombre }}</h3>
                        <p class="text-gray-500 text-sm line-clamp-2 mb-4">
                            {{ $prod->descripcion ?? 'Producto personalizable de alta calidad.' }}</p>
                        <div class="flex items-center justify-between border-t pt-4">
                            <span class="text-2xl font-black text-gray-900">Bs.
                                {{ number_format($prod->precio, 2) }}</span>
                            <a href="https://wa.me/59169608947?text=Hola,%20me%20interesa%20el%20producto:%20{{ urlencode($prod->nombre) }}"
                                target="_blank"
                                class="bg-gray-900 hover:bg-blue-600 text-white p-3 rounded-xl transition-colors">
                ...
                <div class="col-span-1 sm:col-span-2 lg:col-span-3 text-center py-12">
                    ...
                    <p class="text-gray-500">Pronto tendremos más novedades para ti. ¡Vuelve pronto!</p>
                </div>
                @endforelse
            </div>
